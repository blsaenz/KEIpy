module kei_eco
  ...
  REAL(KIND=dbl_kind) :: &
       ...
       parm_sp_kFe,           & ! sphyto iron half sat. coef. (nmol Fe/m3) !mmol actually
       parm_diat_kFe,         & ! diatom iron half sat. coef. (nmol Fe/m3) ! !mmol actually
...
!      diat_mort  = 0.1_dbl_kind   * dps, & !diatom mort rate (1/sec) ! original
 !     diat_mort2 = 0.009_dbl_kind * dps, & !diatom quad mort rate (1/sec/((mmol C/m3)) ! ben test

 !   sp_mort2   = 0.0009_dbl_kind * dps, & !sphyto quad. mort rate (1/sec/((mmol C/m3)) ! original
 !     diat_mort2   = 0.0009_dbl_kind * dps, & !diat quad. mort rate (1/sec/((mmol C/m3)) ! original

    sp_mort    = 0.17_dbl_kind   * dps, & !sphyto mort rate (1/sec) ! CC exp 20 - final
    sp_mort2   = 0.0035_dbl_kind * dps, & !sphyto quad. mort rate (1/sec/((mmol C/m3)),
    diat_mort  = 0.17_dbl_kind   * dps, & !diatom mort rate (1/sec) CC exp 20 - final
...
!       diat_agg_rate_max = 0.2_dbl_kind, & !max agg. rate for diatoms (1/d)
  ...
  !    z_ingest         = 0.2_dbl_kind,  & !zoo ingestion coefficient (non-dim), CC exp 23
...
!      f_z_grz_sqr_diat = 1.5_dbl_kind, &     ! sevrine recommendation, exp 10
      ...
      f_graze_si_remin    = 0.5_dbl_kind      !fraction of diatom Si grazing
                                             !          which is remin ! documentation says this should be 60%
  ...
  !-----------------------------------------------------------------------
  !     SET FIXEDfor N/C, P/C, SiO3/C, Fe/C
...
!***********************************************************************
      ...
      subroutine ecosys_step(kforce,X,fice,albocn,Sref,dt_sec,qsw_ice,hmix,nt,start_year)
        ...
        real(KIND=dbl_kind) :: PAR_out,PAR_in,PAR_avg,dz_ml, KPARdz, dz1
         !real(KIND=dbl_kind) :: PAR_in,PAR_avg,dz_ml, KPARdz, dz1
...
!-----------------------------------------------------------------------
 ...
 !       qsw_eco = kforce(qswins_f_ind)*(c1-albocn) ! edited CC 8/13, calculate for only open water
...
!                PAR_out(k) = PAR_out_tavg(i,j) ! added 1/28 to test
...
!      parm_z_mort2_0      = 0.009_dbl_kind * dps ! kim, CC exp 5 & 21
...
!      parm_z_grz          = 1.3_dbl_kind ! CC exp79 8/5/2023
!      parm_alphaChl       = 0.3_dbl_kind * dps
    parm_alphaChl       = 0.25_dbl_kind * dps
!        parm_alphaChlsp     = 0.22_dbl_kind * dps  ! CC test exp 60
    parm_alphaChlsp     = 0.28_dbl_kind * dps ! original
 !    parm_alphaChldiat   = 0.18_dbl_kind * dps  ! ben playing, CC test exp 60
    parm_alphaChldiat   = 0.25_dbl_kind * dps ! original
...
!-----------------------------------------------------------------------
      ...
      allocate( &
         ...
         PAR_in_tavg(imt,jmt),          PAR_out_tavg(imt,jmt), &
...
!      PAR_in_tavg          = c0    ; cnt = cnt + 1
      PAR_out_tavg         = c0    ; cnt = cnt + 1
...
!       a Q10 value of 3.5 soft bSi (TfuncS) -- says 3.5 but looks like 4?
!
!-----------------------------------------------------------------------
