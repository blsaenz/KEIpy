...
!
! ----------------------------------------------------------------------
	subroutine siesta_create_ice(ice,iis,hi_new,hs_new,af_new,Tair, &
		t_ocn,s_ocn)
	...
	! --------------------------------------------------------------------
		...
		! find new ice grid			
		call siesta_new_grid(hi_new,z_th_min,10.e0,2,1,z_max_ice-sk_z,.false., &
			th_new,id_new,z_new)
		sk_1 = z_new + 1
		sk_z = z_new + z_sk
		int_z = z_new
...
! ----------------------------------------------------------------------
	...
	subroutine siesta_pre_grid(ice,f,m,flooded,c_gl,s_gl, &
...
!			.or. (flooded .ge. fl_max*2.)) then				   
		if (flooded .ge. fl_max) then
		 ...
		 ! snow heat is assumed to be just latent heat of melting + 0.02*t*d
			...
			t_flood = siesta_ice_temp(s_flood,d_mean,heat_mean)

			!call siesta_ice_state(t_flood,s_flood,bs_flood,bd_flood,d_flood, &
			!	bv_flood,heat_flood		
...
! ----------------------------------------------------------------------
	...
	subroutine siesta_ice_remap(ice,f,m,int_z_new,th_new,id_new,t_flood, &
	...
	!				dz_total = dz_total + dz
				...
				if (dz_total .lt. th_new(ii)) then
					...
					jj = 0
					old_layer_bot = c0
					! jj .lt. sk_z so jj never exceeds sk_z after jj=jj+1 (same as siesta_grid.f90).
	...
	!				endif												 
					...
					if (s_mean .le. 0.) then
						...
						print *,'mi: ',mi,'layer: ',iii,'in siesta_env_ice_remap line 363'
					endif								 
...
! **********************************************************************
	...
	subroutine siesta_snow_remap(ice,f,new_snow,z_snow_new,th_new, &
		flooded,melt_drain)
...
! ----------------------------------------------------------------------
	pure subroutine siesta_melt_subl_ice(ice,F_surf,h_top,h_bot,t_ocn, &
	...
	! --------------------------------------------------------------------
			...
			if (basal .and. subl) then
				print *,'Error in siesta_melt_subl_ice: cannot sublime basal ice!'
			else
							...
							if (dz .gt. c0) then
								...
								if (ignore_f) then
									...
									if (subl) then
										...
										q_melt = siesta_ice_heat_melt(ice%t(i),ice%s(i),ice%d(i))
									endif
								...
								if (subl) then
									...
									ice%flux(wf) = ice%flux(wf) + &
										siesta_layer_h20(ice%d(i),ice%s(i))*dh_melt*ice%af  ! report kg melt water -> ocn
									ice%flux(sf) = ice%flux(sf) + &
										siesta_layer_salt(ice%d(i),ice%s(i))*dh_melt*ice%af  ! report kg salt -> ocn
								endif
								...
								if (ignore_f) then
									ice%flux(q_latmelt) = ice%flux(q_latmelt) + &
										siesta_ice_heat_melt(ice%t(i),ice%s(i), &
